...
def find_how_many_pumpkins_are_needed_to_feed_animals(animal_list: list) -> int:
    ...
    """
    return math.ceil(reduce(lambda total, animal: total + (animal.weight_range[0] + animal.weight_range[1]) / 2 * 0.06 * 2 * 90 / 3, filter(lambda animal: animal.diet in ["herbivorous", "omnivorous"], animal_list), 0))
